...
def compute_energy(model, x_state):
    ...
    #constraint = torch.threshold(theta_hat[:, -1], threshold = 0.0, value = 0.0)
    #constraint = torch.threshold(-theta_hat[:, -1], threshold = 0.0, value = 0.0)

    #constraint_bound = soft_bound_constraint(lower_limit = -math.pi, upper_limit = 0.0, eps_rel = 1e-1, stiffness = 1e-0, x = theta_hat[:, -1])
    #constraint = soft_bound_constraint(lower_limit = 0.0, upper_limit = math.pi, eps_rel = 1e-1, stiffness = 1e-0, x = theta_hat[:, -1])
    
    assert(bool(torch.all(constraint_bound >= 0.0).detach().cpu().item()) and True, "No negative values!")
...
def compute_and_save_joint_plot(model, device, dpi, n_one_direction, dir_path_img, fname_img):
    ...
    c = axes[0].pcolormesh(dimX, dimY, theta_hat_1, cmap = 'RdBu', shading = 'gouraud', vmin = rad_min, vmax = rad_max)
    ...
    c = axes[1].pcolormesh(dimX, dimY, theta_hat_2, cmap = 'RdBu', shading = 'gouraud', vmin = rad_min, vmax = rad_max)
...
def compute_and_save_heatmap_plot(model, device, X_state_train, metrics_test, dpi, n_one_direction_heatmap, dir_path_img, fname_img):
    ...
    ax.set_title(
        '\nTrained Neural Network Terminal Energy Landscape [m]\n2D Two-Linkage Robot Inverse Kinematics\n',
...
for j in range(N_ITERATIONS) :
    ...
    '''
    X_state_train_old = X_state_train
    X_state_train = torch.tensor([compute_sample() for _ in range(N_SAMPLES_TRAIN)], dtype = DTYPE_TORCH).to(device)

    diffs.append(torch.sum(torch.norm(X_state_train_old - X_state_train, p = 2, dim = -1), dim = -1))

    if nb_actual_iterations % 1000 == 0 :

        diffs = torch.tensor(diffs)

        print(f"{nb_actual_iterations} sample diffs: mean {torch.mean(diffs):0.2f} sum {torch.sum(diffs):0.2f} max {torch.max(diffs):0.2f}")
    ...
    if nb_actual_iterations % TENSORBOARD_UPDATE == 0 or j == 0 or j == N_ITERATIONS - 1 :
        ...
        if j == N_ITERATIONS - 1 :
            ...
            image_tensor = compute_and_save_heatmap_plot(model, device, X_state_train, metrics_test, heatmap_dpi, n_one_direction_heatmap, dir_path_id_heatmap, heatmap_name)
            
        else :
            
            image_tensor = compute_and_save_heatmap_plot(model, device, X_state_train, metrics_val, heatmap_dpi, n_one_direction_heatmap, dir_path_id_heatmap, heatmap_name)
